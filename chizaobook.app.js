...
Chizaobook_appData = new Object(); // 静态属性
    ...
    Chizaobook_appData.Version = "v1.2.0"; // 版本号
    Chizaobook_appData.Developers = ['KuriCL']; // 开发者
    Chizaobook_appData.update_Time = "2026-07-27 17:48:24"; // 最近维护时间
...
class Chizaobook { // 传入参数
    ...
    selector(){ // 选择器
        ...
        if ((this.packageSource === undefined) || (this.packageSource == null) || (this.packageSource == "") || (this.packageSource == []) || (this.packageSource == [''])) {
            ...
            let judgeType = "Package_source";
            getPackage(packageSource, judgeType);
        }
...
function getPackage(packageSource, judgeType){
            ...
            if ((rq.Request.status == 200) || (rq.Request.status == 304)) {
                ...
                if (judgeType == "Package_source") {
                    ...
                    if (judgeType == "Package"){ // 是数据包
                        ...
                        const result = "判断请求传回数据的类型时";
                        const solution = "可能是数据包或数据源格式错误导致的，请更换数据包或数据源文件。";
                        console.error("\n %c 摛藻(Chizao Book) " + Chizaobook_appData.Version + "%c ❌ERROR!未正确加载!该错误是在" + result + "出现的。（尝试解决方案：" + solution + "）" + "HTTP状态码：" + rq.Request.status + "\n","color: #fadfa3; background: #89303f; padding:5px 0;","background: #ccc3aa; padding:5px 0;"); // 控制台日志 ERROR
                }
